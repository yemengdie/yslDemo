...
app.use(expressJwt({ secret: 'secret' }).unless({
  path: ['/', '/api/user/login', '/api/user/register', '/api/goods/list', '/api/goods/detail', '/api/admin/register', '/api/admin/login', '/api/admin/lunbo'] //除了这些地址，其他的URL都需要验证
}));
...
app.listen(3300);
console.log('后端服务已开启,现在监听3300端口...')
module.exports = app;